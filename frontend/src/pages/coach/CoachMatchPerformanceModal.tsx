...
interface CoachMatchPerformanceModalProps {
    playerId: string;
    playerName: string;
    onClose: () => void;
    onUpdate: () => void;
...
export default function CoachMatchPerformanceModal({
    playerId,
    playerName,
    onClose,
    onUpdate
}: CoachMatchPerformanceModalProps) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setIsSaving(true);
        try {
            await api.post(`/coach/players/${playerId}/matches`, match);
            alert("Match data added successfully! Player statistics have been recalculated.");
            onUpdate();
        } catch (err) {
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="bg-gray-50 p-4 rounded-lg">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input required type="date" value={match.matchDate} onChange={(e) => setMatch({ ...match, matchDate: e.target.value })} className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-purple-500 text-sm" />
                                </div>
                                <div>
                                    <label className="block text-xs font-medium text-gray-600 mb-1">Opponent / Team <span className="text-red-500">*</span></label>
                                    <input required type="text" placeholder="Opponent team name" value={match.opponent} onChange={(e) => setMatch({ ...match, opponent: e.target.value })} className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-purple-500 text-sm" />
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div className="space-y-4">
                                <h3 className="text-sm font-semibold text-purple-700 bg-purple-50 p-2 rounded uppercase tracking-wider">Batting</h3>
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        <label className="block text-xs font-medium text-gray-600 mb-1">Runs Scored</label>
                                        <input type="number" min="0" value={match.runsScored} onChange={(e) => setMatch({ ...match, runsScored: Number(e.target.value) })} className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-purple-500 text-sm" />
                                    </div>
                                    <div>
                                        <label className="block text-xs font-medium text-gray-600 mb-1">Balls Faced</label>
                                        <input type="number" min="0" value={match.ballsFaced} onChange={(e) => setMatch({ ...match, ballsFaced: Number(e.target.value) })} className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-purple-500 text-sm" />
                                    </div>
                                    <div>
                                        <label className="block text-xs font-medium text-gray-600 mb-1">Fours (4s)</label>
                                        <input type="number" min="0" value={match.fours} onChange={(e) => setMatch({ ...match, fours: Number(e.target.value) })} className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-purple-500 text-sm" />
                                    </div>
                                    <div>
                                        <label className="block text-xs font-medium text-gray-600 mb-1">Sixes (6s)</label>
                                        <input type="number" min="0" value={match.sixes} onChange={(e) => setMatch({ ...match, sixes: Number(e.target.value) })} className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-purple-500 text-sm" />
                                    </div>
                                    <div className="col-span-2 flex items-center mt-2">
                                        <input type="checkbox" id="isDismissed" checked={match.isDismissed} onChange={(e) => setMatch({ ...match, isDismissed: e.target.checked })} className="h-4 w-4 text-purple-600 focus:ring-purple-500 rounded" />
