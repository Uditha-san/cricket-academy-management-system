import { Calendar, Wrench, ArrowLeft, Lock } from 'lucide-react';
    ...
    return (
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 min-h-[calc(100vh-4rem)] flex flex-col">
            <div className="mb-12">
                ...
                {isGuest && (
                    <div className="mt-3 inline-flex items-center gap-2 bg-orange-50 border border-orange-200 text-orange-700 text-sm px-4 py-2 rounded-lg">
                        <Lock className="w-4 h-4" />
                        You're browsing as a guest. Court booking is available; machine rental requires academy membership.
                    </div>
                )}
            </div>
            ...
            <div className="flex-1 flex items-start justify-center">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 w-full max-w-4xl">
                    ...
                    {/* Rent a Machine — blocked for guests */}
                    <button
                        onClick={() => { if (!isGuest) onNavigate('machine-rental'); }}
                        disabled={isGuest}
                        className={`group relative bg-white rounded-3xl p-8 shadow-sm transition-all duration-300 overflow-hidden text-left border ${isGuest
                            ? 'border-gray-200 opacity-60 cursor-not-allowed'
                            : 'border-gray-100 hover:border-blue-500 hover:shadow-2xl hover:-translate-y-1'
                            }`}
                    >
                        <div className="absolute inset-0 bg-gradient-to-br from-blue-50/50 to-transparent opacity-0 group-hover:opacity-100 transition-opacity" />
                        <div className="relative z-10">
                            <div className={`w-20 h-20 rounded-2xl flex items-center justify-center mb-6 shadow-inner transition-transform duration-300 ${isGuest
                                ? 'bg-gray-100 text-gray-400'
                                : 'bg-blue-100 text-blue-600 group-hover:scale-110 group-hover:-rotate-3'}`}>
                                {isGuest ? <Lock className="w-10 h-10" /> : <Wrench className="w-10 h-10" />}
                            </div>
                            ...
                            {isGuest ? (
                                <div className="mt-8 flex items-center gap-2 text-orange-600 font-semibold text-sm">
                                    <Lock className="w-4 h-4" />
                                    Academy members only — register to unlock
                                </div>
                            ) : (
